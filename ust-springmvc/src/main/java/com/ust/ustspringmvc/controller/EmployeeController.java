...
public class EmployeeController {
	@Autowired
	private EmployeeService service;   //autowired the service object
	...
	@GetMapping("/login")     //will display the login page
	...
	@GetMapping("/register")
	public String registerPage(){    //to go to the form url   and will display register page
		return "register";
		}
	
	@PostMapping("/login")  //it is possible as we are accesing different method from same path login
	...
	request.getSession().setAttribute("emp", employee); //to create session and to set session attribute 
	    return "home";
	}
	...
	public String register(Employee employee, ModelMap map) {
		...
		if(service.registerEmployee(employee)) {
			map.addAttribute("msg", "Registered the Employee successfully");
		} else {
	...
	public String logout(HttpSession session, ModelMap map) {
		session.invalidate();
		map.addAttribute("msg", "Successfull Logout");
		return "login";
	}
}
